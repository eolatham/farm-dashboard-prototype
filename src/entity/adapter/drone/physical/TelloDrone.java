...
public class TelloDrone extends MultiRotorDrone {
  private final int maxGoto = 500, minGoto = -500, minDist = 20, maxSpeed =
    100, minSpeed = 10, maxDegrees = 360, minDegrees = 1;
  ...
  public void increaseAltitude(int up) throws IOException {
    if (up <= minDist) this.controller.sendCommand("up " + minDist); else if (
      up > maxDist
    ) {
  ...
  public void decreaseAltitude(int down) throws IOException {
    ...
    if (down <= minDist) this.controller.sendCommand(
        "down " + minDist
  ...
  public void flyForward(int front) throws IOException {
    if (front <= minDist) this.controller.sendCommand(
        "forward " + minDist
  ...
  public void flyBackward(int back) throws IOException {
    if (back <= minDist) this.controller.sendCommand(
        "back " + minDist
  ...
  public void flyLeft(int left) throws IOException {
    if (left <= minDist) this.controller.sendCommand(
        "left " + minDist
  ...
  public void flyRight(int right) throws IOException {
    if (right <= minDist) this.controller.sendCommand(
        "right " + minDist
  ...
  public void turnCW(int degrees) throws IOException {
    if (degrees <= minDegrees) this.controller.sendCommand(
        "cw " + minDegrees
  ...
  public void gotoXYZ(int x, int y, int z, int speed) throws IOException {
    ...
    if (x <= maxGoto && x >= minGoto && y <= maxGoto && y >= minGoto) {
      ...
      System.out.println(
        String.format("go %1$d %2$d %3$d %4$d", maxGoto, -partialY, z, speed)
      );
      ...
      (x < minGoto && y <= maxGoto && y >= minGoto) ||
      (
        ((x > maxGoto || x < minGoto) && (y > maxGoto || y < minGoto)) &&
        (Math.abs(x) > Math.abs(y)) &&
        (x < minGoto)
      )
    ) {
      ...
      System.out.println(
        String.format("go %1$d %2$d %3$d %4$d", minGoto, -partialY, z, speed)
      );
      ...
      System.out.println(
        String.format("go %1$d %2$d %3$d %4$d", partialX, minGoto, z, speed)
      );
      ...
      System.out.println(
        String.format("go %1$d %2$d %3$d %4$d", partialX, maxGoto, z, speed)
      );
      ...
      if (x > maxGoto && y < minGoto) {
        System.out.println(
          String.format("go %1$d %2$d %3$d %4$d", maxGoto, maxGoto, z, speed)
        );
        ...
        System.out.println(
          String.format("go %1$d %2$d %3$d %4$d", minGoto, minGoto, z, speed)
        );
        ...
        System.out.println(
          String.format("go %1$d %2$d %3$d %4$d", maxGoto, minGoto, z, speed)
        );
        ...
        System.out.println(
          String.format("go %1$d %2$d %3$d %4$d", minGoto, maxGoto, z, speed)
        );
   ...
   * @throws IOException
   */
  public void gotoMissionPadXY(int x, int y, int speed, String ID)
    throws IOException {
    int z = 0;
    double slope = (double) y / x;
    //System.out.println(slope);
   ...
   * @throws IOException
   */
  public void flyCurveMissionPad(
    int x1,
    int y1,
    int z1,
    int x2,
  ...
  public void setSpeed(int speed) throws IOException {
    if (speed <= minSpeed) this.controller.sendCommand(
        "speed " + minSpeed
  ...
  public int getTemp() throws IOException {
    try {
      ...
      int temp2 = Integer.parseInt(
        arrayOfStr[1].substring(0, arrayOfStr[1].length() - 1)
      );
  ...
  public int getTOF() throws IOException {
    try {
      ...
      return (
        (
          Integer.parseInt(timeOfFlight.substring(0, timeOfFlight.length() - 2))
        ) /
        10
      );
    } catch (NumberFormatException e) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    while (
      !command.equals("end") && command != null && !command.trim().isEmpty()
    ) {
